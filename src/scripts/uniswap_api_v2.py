...
def swap_exact_eth_amount_for_token(user_account, token_out, amount_in):
    tx = UniswapV2DMTransact[-1].swapExactETHForTokens(token_out, {"from": user_account, "value": amount_in})
...
def swap_eth_for_exact_token_amount(user_account, token_out, amount_in):
    ...
    _, expected_amount_out = UniswapV2DMTransact[-1].getAmountOut(amount_in, weth_token, token_out)
    tx = UniswapV2DMTransact[-1].swapETHForExactTokens(expected_amount_out, token_out, {"from": user_account, "value": amount_in})
...
def swap_exact_token_amount_for_eth(user_account, token_in, amount_in):
    ...
    tx = UniswapV2DMTransact[-1].swapExactTokensForETH(amount_in, token_in, {"from": user_account})


def swap_token_for_exact_eth_amount(user_account, token_in, amount_in, slippage):
    weth_token = config['networks'][network.show_active()]['weth_token']
    interface.IERC20(token_in).approve(UniswapV2DMTransact[-1].address, amount_in, {"from": user_account})
    amount_out, new_amount_in = UniswapV2DMTransact[-1].getAmountIn(amount_in, weth_token, token_in)
    tx = UniswapV2DMTransact[-1].swapTokensForExactETH(new_amount_in, slippage * amount_out, token_in, {"from": user_account})


def swap_exact_token_amount_for_token(user_account, token_in, token_out, amount_in):
    interface.IERC20(token_in).approve(UniswapV2DMTransact[-1].address, amount_in, {"from": user_account})
    tx = UniswapV2DMTransact[-1].swapExactTokensForTokens(amount_in, token_in, token_out, {"from": user_account})


def swap_token_for_exact_token_amount(user_account, token_in, token_out, amount_in, slippage):
    interface.IERC20(token_in).approve(UniswapV2DMTransact[-1].address, amount_in, {"from": user_account})
    new_amount_in, amount_out = UniswapV2DMTransact[-1].getAmountOut(amount_in, token_in,  token_out)
    print(new_amount_in / 10**18)
    print(amount_out / 10**18)
    tx = UniswapV2DMTransact[-1].swapTokensForExactTokens(amount_in, slippage * amount_out, token_in, token_out, {"from": user_account})
...
def main():
    ...
    #if network.show_active()[-4:] == 'fork':
    router02 = config['networks'][network.show_active()]['uniswapV2Router02']
    deploy_uniswap_v2_actor(PERSON, router02)
    print(UniswapV2DMTransact[-1].address)
